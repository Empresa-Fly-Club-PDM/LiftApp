package com.jder00138218.liftapp.ui.users.user.updateuserexercise.viewmodel
...
class UpdateUserExerciseViewModel (private val exerciseRepository: ExerciseRepository): ViewModel(){
    ...
    fun onUpdate(navController: NavHostController, context: Context, exerciseid:Int?) {
        if (!validateData()) {
            _status.value = AdminUpdateExerciseUIStatus.ErrorWithMessage(context.getString(R.string.verificar_information))
    ...
    companion object {
        val Factory = viewModelFactory {
            initializer {
                ...
                UpdateUserExerciseViewModel(app.exerciseRepository)
            }
        }
    }
}